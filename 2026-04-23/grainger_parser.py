...
class GraingerParser:
    ...
    def parse_item(self, pdp_url, html_content):
        try:
            ...
            config_data     = self.get_nested(data, ["product", "configData", sku])
            ...
            # Item Name
            item_name = self.get_nested(product_details, ["primaryNoun"]) or ""
            ...
            # RoHS / Reach — compliance messages
            compliance_msgs = self.get_nested(product_details, ["complianceMessages"]) or {}
            if isinstance(compliance_msgs, dict) and compliance_msgs:
                # Strip HTML tags from compliance text
                raw_rohs = " | ".join(compliance_msgs.values())
                rohs_reach = re.sub(r'<[^>]+>', '', raw_rohs).strip()
            else:
                rohs_reach = ""
